    ...
    def test_easy_routes_api_and_no_dominant_technical_terms(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("radar_web.app.read_scheduler_status", return_value={"status": "NO_DATA"}):
                ...
                api_day_packet = client.get(f"/api/easy/days/{run_id}/model-packet")
                html_brief = client.get("/easy/latest/brief?lang=it")
                html_packet = client.get("/easy/latest/model-packet?lang=it")
                html_day_brief = client.get(f"/easy/days/{run_id}/brief?lang=it")
                html_day_packet = client.get(f"/easy/days/{run_id}/model-packet?lang=it")
                actions_html = client.get("/actions?lang=it")
        ...
        self.assertEqual(api_day_packet.status_code, 200)
        self.assertEqual(latest_brief.status_code, 200)
        self.assertEqual(latest_packet.status_code, 200)
        self.assertEqual(html_brief.status_code, 200)
        self.assertEqual(html_packet.status_code, 200)
        self.assertEqual(html_day_brief.status_code, 200)
        self.assertEqual(html_day_packet.status_code, 200)
        self.assertEqual(expert.status_code, 200)
        self.assertIn("text/html", html_brief.headers["content-type"])
        self.assertIn("text/html", html_packet.headers["content-type"])
        self.assertIn("application/json", latest_brief.headers["content-type"])
        self.assertIn("application/json", latest_packet.headers["content-type"])
        ...
        self.assertIn('href="/easy/latest/brief?lang=it"', actions_html.text)
        self.assertIn("Cosa e successo", detail.text)
        self.assertIn("Riepilogo umano", html_brief.text)
        self.assertIn("HAG preservato", html_brief.text)
        self.assertIn("Solo manuale", html_brief.text)
        self.assertNotIn('"brief_type"', html_brief.text)
        self.assertIn("Pacchetto modello IA", html_packet.text)
        self.assertIn("Azioni vietate", html_packet.text)
        self.assertIn("<pre", html_packet.text)
        self.assertNotIn("navigator.clipboard", html_packet.text)
        self.assertNotIn("Set-Clipboard", html_packet.text)
